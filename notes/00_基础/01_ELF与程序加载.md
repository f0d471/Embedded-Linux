...
> **ELF 是可执行文件的包装规格。** 一个程序文件里光有机器码没用，还得写清楚：
> 这是给哪种 CPU 的、从哪条指令开始跑、哪一段是代码哪一段是数据、要不要别人的库。
> 这些说明和机器码一起放在文件里，按一套固定格式写——这套格式就叫 ELF。

### 你是从哪儿撞见它的

你 `file a.out`，它回你一句 `ELF 32-bit LSB executable, ARM`。
或者更常见的：你把在电脑上编好的程序拷到板子上，敲下去，
板子回你 `Exec format error`——**文件明明在那儿，权限也给了，就是不认。**

大部分人这时候的反应是重编一遍、换个 `-static` 试试。但这条报错的意思其实很具体：
**内核翻开这个文件的头几十个字节，看到的东西它不认识，所以拒绝往下走。**
要看懂这句话，就得知道那头几十个字节里写的是什么。

### 为什么一个文件非要长这样不可

上一篇结束时你拿到了一个链接完的文件。现在的问题是：**这个文件要给好几拨人看。**

    链接器要读它      我得知道哪块是代码、哪块是数据，才能和别的文件拼起来
    内核要读它        我得知道把哪一段搬到内存哪个地址、给读还是给写
    调试器要读它      我得知道这个地址对应源码第几行、这个函数叫什么名字

这几拨人关心的完全不是一回事，但他们读的是同一个文件。
**所以文件里必须自带"说明书"，而且说明书的写法要全世界统一**，
不然换个工具就读不懂了。ELF 就是这份统一写法。

麻烦在于，链接器和内核想要的**切法**根本不同：

    链接器要细     按用途切：代码归代码、只读数据归只读数据、零初始化变量归一堆
                   因为它要把好几个文件的同类内容拼到一起
    内核要粗       按权限切：能读能执行的划一片，能读能写的划另一片
                   因为它是按内存页来映射的，一次摆一大块，还得页对齐

一份内容，两种切法都得要，怎么办？**答案是两张目录并存**：

    section（节）      按用途切的那张，给链接器和调试器用
    segment（段）      按权限切的那张，给内核用，每项叫一个 program header

这不是设计得啰嗦，是两个读者天生要的粒度不一样。
本篇第三节会实测一个很干脆的证据：**把整张 section 表删光，程序照跑不误；
而只改 2 个字节让 segment 数变成 0，立刻 `Exec format error`。**
谁是加载时真正被读的那张表，这个实验一次说清。

### 加载到底是怎么发生的

`Exec format error` 那一关过了之后，剩下的事是这样的：

    内核     按 LOAD segment 把文件映射进一个新的地址空间（注意是映射，不是读进来）
    ld.so    如果这程序要用动态库，内核先把这个"动态加载器"拉起来，由它去找齐 .so
    _start   真正的第一条指令在这儿，是 C 运行库提供的
    libc     初始化全局变量、准备好 argc/argv/环境变量
    main     现在才轮到你写的代码

**`main` 从来就不是第一条指令。** 知道这条链，以后"程序没进 main 就挂了"
才有地方可查。
...
1. **ELF 是文件格式，不是"可执行程序"的同义词。** `.o`、`.so`、可执行文件都是 ELF；
   能不能跑，看 ELF 头里写的类型和目标架构。
2. **一份内容两张目录：section 按用途切给链接器，segment 按权限切给内核。**
   两者不是一一对应，谁也不需要对方那张。
3. **加载时内核只认 program header 里的 `LOAD` 项**，它写明把文件的哪一段摆到哪个
   虚拟地址、给什么权限。section 表这时候一眼都不会看。
4. **`LOAD` 行里 `FileSiz` 是文件里占几字节，`MemSiz` 是内存里要几字节。**
   多出来的那截由内核补零，`.bss` 就藏在这个差里（差值还含对齐，不等于 `.bss` 的
   精确大小）。
5. **`.bss` 只记大小，不在文件里存那一堆零。** 所以一个大零数组能让程序吃掉内存，
   文件却几乎没变大；但"文件省了"不等于"运行时免费"。
6. **`main` 不是第一条指令。** 动态链接程序的顺序是：内核映射 → `ld.so` 找齐库 →
   `_start` → libc 初始化 → `main`。
7. **`Exec format error` 的意思是"内核不认这个文件"，不是"文件坏了"。**
   先 `file` 或 `readelf -h` 看架构和类型，别急着重编。

### 看到什么，就先想到什么

| 看到的东西 | 第一反应 |
|---|---|
| `readelf -h` / `-S` / `-l` | 分别看 ELF 头、section 表、program header 表（`LOAD` 在这张里） |
| `Exec format error` | 内核读头部没认出来；`file` 看架构对不对、类型是不是可执行 |
| 文件明明在，执行却说"找不到" | 找不到的多半不是它，是 `readelf -l` 里那个解释器（`ld.so`）路径在板子上不存在 |
| 大数组让内存涨了，文件却没大多少 | 它进了 `.bss`；再往下要分清文件大小、虚拟地址空间、实际驻留内存三件事 |
