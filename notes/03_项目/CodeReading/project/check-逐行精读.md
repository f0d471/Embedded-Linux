...
`project/` 的判据。在 WSL 里 `bash check.sh` 跑一次，输出 29 条 PASS/FAIL，
末尾给计数，退出码为 0 表示全绿。
...
`sed -i '/{ "business",/d'` 删掉匹配这个形状的整行，也就是层表第 26 行。
...
## 11. 第 6 组：日志落文件（第 104 到 162 行）

这一组是 02 章的产出，十一条判据分三段。

### 11.1 正判据（第 104 到 122 行）
...
| 退出码 1 | 打不开时静默降级继续跑 | — |
| 具体系统错误 | `errno` 在项目错误码转换时丢失 | 其他 errno 是否正确 |
| 空路径只有参数错误 | 旧 errno 被误带到没有系统调用失败的分支 | — |
...
两条错误文本判据都先用 `LC_ALL=C` 固定 `strerror` 的语言，再用 `sed` 去掉
日志宏自动添加的文件名和行号前缀，比较完整的错误正文。这样源码增删行不会让判据
失效，同时也不是只检查某个词“出现过”的假绿判据。

### 11.2 stderr 无缓冲那条（第 124 到 133 行）
...
而"日志走 stderr"正是这套设计的地基（`common.h` 第 27 到 37 行），
所以它必须有自己的守门人。
...
第三次注错保留 `ERR_IO` 和退出码 1，只把 `*os_errno = errno` 改成 0。
程序仍然知道“IO 失败”，却说不出为什么失败；错误正文从
`device io failed (No such file or directory)` 退化成 `device io failed`。
这证明原有退出码判据覆盖不到新增的 errno 通路。
...
第二、三次注错前都先从 `$SRC` 还原，避免多个错误叠在一起。
后一个判据红了也必须能说清是哪一处造成的。**一次只坏一个地方**，
这是本脚本里每一组注错都遵守的规矩。
...
| 9 | 104-133 | 第 6 组，8 条或 7 条加 1 条 SKIP | 在 `$W/run.log` 里攒下 26 行日志，并核对两类错误文本 |
| 10 | 135-162 | 第 6r / 6r2 / 6r3 三组，各 1 条 | `common.c` 被改坏三次，各自还原 |
| 11 | 164-166 | 打计数，定退出码 | trap 触发，删掉临时目录 |

第 4、6、7、10 步包含"改坏又还原"。还原用的是从 `$SRC` 复制，
...
**判据总数 29 是"跑得到的条数"，不是"写了几条"。**
第 2 组在没装交叉工具链的机器上是 1 条 SKIP 而不是 5 条 PASS，
那种情况下末尾会是 `PASS=25 FAIL=0 SKIP=1`；没装 strace 再少一条 PASS、
多一条 SKIP。
...
| 谁 | 关系 |
|---|---|
| 人 | `bash check.sh`，看 `PASS=29 FAIL=0 SKIP=0` |
...
| `main.c` 第 26 行 | `{ "business",` 这个字面形状 |
| `main.c` 第 73 行 | 环境变量名 `LOG_FILE` |
| `common.c` 第 35 行 | `O_WRONLY`、`O_CREAT`、`O_APPEND` 三个 flag 拼写与空格 |
| `common.c` 第 41 行 | `if (dup2(fd, STDERR_FILENO) < 0)` 这个字面形状 |
| `common.c` 第 37 行 | `*os_errno = errno;` 这个字面形状 |
